...
def process_frame(frame, lower, upper):
    ...
    if len(contours) > 0:
        ...
        area = cv2.contourArea(largest_contour)  # if the num of contours is > than 0 then pick the biggest one
    else:
        area = 0
    #if area > 200:
    if area > 200:  # makes sure than the comp vision doesn't detect super small things
        detection = True
